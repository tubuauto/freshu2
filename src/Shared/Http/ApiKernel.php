...
final class ApiKernel
{
    ...
    public function handle(Request $request): array
    {
        ...
        $walletService = new WalletService($pdo, $walletRepository, $this->appConfig['currency']);
        $rechargeRepository = new RechargeRepository($pdo);
        $rechargeService = new RechargeService($pdo, $rechargeRepository, $walletService, $this->appConfig);

        $couponRepository = new CouponRepository($pdo);
        $couponService = new CouponService($pdo, $couponRepository);

        $orderRepository = new OrderRepository($pdo);
        $orderService = new OrderService($pdo, $orderRepository, $walletService, $couponService, $this->appConfig['currency']);
        ...
        $router = new Router();
        $routeRegistrar = require dirname(__DIR__, 3) . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'routes.php';
