...
- Default UI language: Simplified Chinese (zh-CN)
- Default business language: Chinese
- Currency: CAD
- UI should display money in CAD
- All money values in the database must be stored as integer cents
- Never use float or decimal as the source of truth for money
- All calculations must use integer cents
- Prefer field names like:
  - amount_cents
  - total_amount_cents
  - payable_amount_cents
  - withdrawable_balance_cents
  - non_withdrawable_balance_cents
